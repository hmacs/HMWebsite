    ...
    public function main()
    {
        ...
        foreach ($results as $k => $result) {
            ...
            $response->rows [$i] ['cell'] = array(
                ...
                $result ['date_added'],
                $result ['section'],
                (is_file($result ['path']) ? '<a target="_blank" title="'.$this->language->get('text_download').'" href="'.$this->html->getSecureUrl('tool/files/download', '&filename='.urlencode($result ['name'])).'&attribute_type='.$result['section'].'&attribute_id='
                    .$result['section_id'].'">'.$result ['name'].'</a>' : ''),
            );
