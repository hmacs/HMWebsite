...
function getProperty($var)
{
    ...
    if ($var == 'shiptimestamp') {
        Return mktime(10, 0, 0, date("m"), date("d") + 4, date("Y"));
    }
    if ($var == 'tag_readytimestamp') {
        Return mktime(10, 0, 0, date("m"), date("d") + 4, date("Y"));
    }
    if ($var == 'tag_latesttimestamp') {
        Return mktime(15, 0, 0, date("m"), date("d") + 4, date("Y"));
    }
    ...
    if ($var == 'locatoraddress') {
        Return array(
            array('StreetLines' => '240 Central Park S'),
            'City'                => 'Austin',
    ...
    if ($var == 'holdcontactandlocation') {
        Return array(
            ...
            'Address' => array(
                'StreetLines'         => array('13085 Worldgate Dr '),
                'City'                => 'Herndon',
                'StateOrProvinceCode' => 'VA',
                'PostalCode'          => '20170',
                'CountryCode'         => 'US',
...
function trackDetails($details, $spacer)
{
    foreach ($details as $key => $value) {
        ...
        if (is_array($value) || is_object($value)) {
            ...
            echo '<td>'.$spacer.$key.'</td><td>'.$value.'</td>';
        }
        echo '</tr>';
    }
}
